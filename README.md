...
flowchart LR
    ...
    MPET --> SUVR[5. SUVR and ROI statistics]
...
普通 DICOM 转换、SUVR 和表格输出建议在 Slicer Python 中安装：
...
2. `Reference mask`：选择参考区 mask，例如 cerebellum GM。
3. `SUVR output`：设置输出名。
4. 可选填写 `Optional save path`。
5. 点击 `Create SUVR image`。

SUVR 计算方式为：

```text
SUVR = PET / mean(PET within reference mask)
```
...
10. `SynthMorph backend`：推荐 `WSL GPU`。
11. 点击 `Check GAAIN SynthMorph backend`。
12. 点击 `Run GAAIN AV45 SUVR`。
...
7. 将 cerebellum 和 ctx mask 对齐到标准空间 PET。
8. 生成 cerebellum 参考区 SUVR。
9. 输出 ctx SUVR 统计表。
