...
def _patch_neurite_keras3_layers():
    """Restore private dtype attributes expected by older neurite layers."""
    for name in ('HyperConvFromDense', 'HyperConv2DFromDense', 'HyperConv3DFromDense'):
        cls = getattr(ne.layers, name, None)
        if cls is not None and not hasattr(cls, '_compute_dtype'):
            cls._compute_dtype = property(lambda self: self.compute_dtype)


_patch_neurite_keras3_layers()


def _feature_barycenter(features, axes, normalize=True, shift_center=True, dtype=None):
    """Compute per-channel spatial barycenters for feature maps."""
    features = tf.convert_to_tensor(features)
    dtype = dtype or features.dtype
    weights = tf.cast(features, dtype)
    axes = tuple(axes)
    denominator = tf.reduce_sum(weights, axis=axes)
    denominator = tf.maximum(denominator, tf.cast(K.epsilon(), dtype))
    rank = len(weights.shape)
    centers = []

    for axis in axes:
        size = tf.shape(weights)[axis]
        coord = tf.cast(tf.range(size), dtype)
        if normalize:
            coord = coord / tf.maximum(tf.cast(size - 1, dtype), tf.cast(1, dtype))
            if shift_center:
                coord = coord - tf.cast(0.5, dtype)
        elif shift_center:
            coord = coord - (tf.cast(size - 1, dtype) * tf.cast(0.5, dtype))

        shape = [1] * rank
        shape[axis] = size
        coord = tf.reshape(coord, shape)
        numerator = tf.reduce_sum(weights * coord, axis=axes)
        centers.append(numerator / denominator)

    return tf.stack(centers, axis=-1)
...
class VxmAffineFeatureDetector(tf.keras.Model):
    ...
    def __init__(self,
        ...
        def constant_matrix(mat, like):
            mat = np.asarray(mat, dtype='float32')
            return KL.Lambda(
                lambda x, m=mat: tf.tile(
                    tf.expand_dims(tf.cast(tf.constant(m), x.dtype), 0),
                    [tf.shape(x)[0], 1, 1],
                ),
                output_shape=tuple(mat.shape),
            )(like)

        def cen(shape):
            mat = np.eye(num_dim + 1)
            mat[:-1, -1] = -0.5 * (shape - 1)
            return constant_matrix(mat, inp_1)
        ...
        # Barycenters. Keras 3 cannot infer these Lambda output shapes.
        prop_b = dict(axes=range(1, num_dim + 1), normalize=True, shift_center=True, dtype=dtype)
        cen_shape = (num_feat, num_dim)
        cen_1 = KL.Lambda(lambda x: _feature_barycenter(x, **prop_b) * shape_full, output_shape=cen_shape)(feat_1)
        cen_2 = KL.Lambda(lambda x: _feature_barycenter(x, **prop_b) * shape_full, output_shape=cen_shape)(feat_2)

        # Channel weights.
        axes = range(1, num_dim + 1)
        pow_shape = (num_feat,)
        pow_1 = KL.Lambda(lambda x: tf.reduce_sum(x, axes), output_shape=pow_shape)(feat_1)
        pow_2 = KL.Lambda(lambda x: tf.reduce_sum(x, axes), output_shape=pow_shape)(feat_2)
        pow_1 /= KL.Lambda(lambda x: tf.reduce_sum(x, axis=-1, keepdims=True), output_shape=(1,))(pow_1)
        pow_2 /= KL.Lambda(lambda x: tf.reduce_sum(x, axis=-1, keepdims=True), output_shape=(1,))(pow_2)
        weights = pow_1 * pow_2
        ...
        if weighted:
            fit_1 = (*fit_1, weights)
            fit_2 = (*fit_2, weights)
        aff_shape = (num_dim, num_dim + 1)
        aff_1 = KL.Lambda(lambda x: utils.fit_affine(*x), output_shape=aff_shape)(fit_1)
        aff_2 = KL.Lambda(lambda x: utils.fit_affine(*x), output_shape=aff_shape)(fit_2)
        aff_1 = KL.average((layers.InvertAffine()(aff_2), aff_1))

        # Remove scaling and shear.
        if rigid:
            aff_1 = KL.Lambda(utils.affine_matrix_to_params, output_shape=(num_dim * (num_dim + 1),))(aff_1)
        ...
        if return_trans_to_mid_space:
            square_shape = (num_dim + 1, num_dim + 1)
            aff_1 = KL.Lambda(utils.make_square_affine, output_shape=square_shape)(aff_1)
            aff_1 = KL.Lambda(tf.linalg.sqrtm, output_shape=square_shape)(aff_1)

            aff_2 = KL.Lambda(utils.make_square_affine, output_shape=square_shape)(aff_2)
            aff_2 = KL.Lambda(tf.linalg.sqrtm, output_shape=square_shape)(aff_2)
...
class HyperVxmJoint(tf.keras.Model):
    ...
    def __init__(self,
        ...
        *hyp_inp, full_1, full_2 = input_model.outputs
        ...
        def scale(fact):
            mat = np.diag((*[fact] * num_dim, 1))
            return constant_matrix(mat, full_1)
