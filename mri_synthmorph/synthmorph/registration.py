...
def register(arg):
    ...
    if not arg.weights:
        arg.weights = [script_dir / filename for filename in weights[arg.model]]
    else:
        arg.weights = [pathlib.Path(filename) for filename in arg.weights]
    for filename in arg.weights:
        if not pathlib.Path(filename).is_file():
            sf.system.fatal(f'model weights not found: {filename}')
    ...
    print('SynthMorph weights: ' + ', '.join(pathlib.Path(f).name for f in arg.weights))
