...
{b}DESCRIPTION{n}
        ...
        {b}-m{n} {u}method{n}
                ...
                to {default['method']}. Choose bspline or linear for scalar
                images and nearest for label (segmentation) maps. Bspline is a
                CPU-based cubic B-spline resampler.
...
def setup_tensorflow(use_gpu, verbose=False, threads=None):
    """Configure TensorFlow before importing SynthMorph registration code."""
    patch_python312_inspect()
    gpu = os.environ.get('CUDA_VISIBLE_DEVICES', '0')
    os.environ['CUDA_VISIBLE_DEVICES'] = gpu if use_gpu else ''
    ...
    if use_gpu:
        ...
        for device in devices:
            try:
                ...
                pass
        if not devices:
            print('WARNING: -g was requested, but TensorFlow did not detect a GPU.', file=sys.stderr)


def patch_python312_inspect():
    """Restore inspect.getargspec for older neurite on Python 3.12."""
...
def patch_keras_loss_aliases(tf):
    """Restore Keras 2 loss function aliases expected by neurite/voxelmorph."""
    ...
    for name, function in aliases.items():
        if not hasattr(losses, name):
            setattr(losses, name, function)


def bspline_transform(image, trans, header_only=False, fill=0):
    """Apply an affine or SynthMorph warp using cubic B-spline interpolation."""
    if header_only:
        return image.transform(trans, resample=False, fill=fill)
    if isinstance(trans, sf.Warp):
        return bspline_warp(image, trans, fill=fill)
    if isinstance(trans, sf.Affine):
        return bspline_affine(image, trans, fill=fill)
    raise ValueError(f'unsupported transform type for bspline interpolation: {type(trans).__name__}')


def bspline_warp(image, warp, fill=0):
    disp = warp.convert(sf.Warp.Format.disp_crs, copy=False)
    coords = target_voxel_grid(disp.baseshape) + disp.framed_data.astype(np.float32, copy=False)
    sampled = sample_bspline(image.framed_data, coords, fill=fill)
    return image.new(sampled, geometry=disp.target)


def bspline_affine(image, affine, fill=0):
    affine = affine.copy()
    ...
    matrix = affine.inv().matrix.astype(np.float32, copy=False)
    grid = target_voxel_grid(target_geom.shape)
    flat = grid.reshape(-1, 3).T
    mapped = matrix[:3, :3] @ flat + matrix[:3, 3:]
    coords = mapped.T.reshape((*target_geom.shape, 3))
    sampled = sample_bspline(image.framed_data, coords, fill=fill)
    return image.new(sampled, target_geom)


def target_voxel_grid(shape):
    axes = [np.arange(dim, dtype=np.float32) for dim in shape]
    return np.stack(np.meshgrid(*axes, indexing='ij'), axis=-1)


def sample_bspline(source, coords, fill=0):
    from scipy.ndimage import map_coordinates

    if source.ndim != 4:
        raise ValueError(f'source data must be 4D, but got input of shape {source.shape}')

    data = source.astype(np.float32, copy=False)
    sample_coords = [coords[..., axis] for axis in range(3)]
    sampled = np.empty((*coords.shape[:-1], data.shape[-1]), dtype=np.float32)
    for frame in range(data.shape[-1]):
        sampled[..., frame] = map_coordinates(
            data[..., frame],
            sample_coords,
            order=3,
            mode='constant',
...
if arg.command == 'register':
    ...
    # TensorFlow setup. This must happen before importing SynthMorph networks.
    setup_tensorflow(arg.gpu, verbose=arg.verbose, threads=arg.threads)
...
if arg.command == 'apply':
    ...
    for inp, out in zip(arg.pairs[::2], arg.pairs[1::2]):
        image = sf.load_volume(inp)
        if arg.method == 'bspline':
            moved = bspline_transform(image, trans, header_only=arg.header_only, fill=arg.fill)
        else:
            prop = dict(method=arg.method, resample=not arg.header_only, fill=arg.fill)
            moved = image.transform(trans, **prop)
        moved.astype(arg.type).save(out)
